function togglePassword() {
        ...
        function moveToNext(current, index) {
            current.value = current.value.toUpperCase().replace(/[^A-Z0-9]/g, '');
            ...
            verifyBtn.disabled = !Array.from(inputs).every(i => i.value.length === 1);
        }

        function submitSerialKey() {
            const key = Array.from(inputs).map(i => i.value).join('');
            if (key.length === 5) {
                window.location.href = "/login?recover=1&key=" + key;
            }
        }
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            inputs.forEach((input, idx) => {
                input.addEventListener('input', () => moveToNext(input, idx));
            });
        });
