    ...
    let Countdown = "00:00";
    let Consumption = "00 m3";
    const IPAdressESP = "http://192.168.1.234";

    async function fetchconsumption() {
      const js = await fetch("https://script.google.com/macros/s/AKfycby5ytRnbEPqiN9wSgPinSHAxKQD7iCQN9GZ5fZJuA6AobgpeApIZ1LFNKHUmN5dN799Wg/exec?action=getConsumption");
      const c = await js.json();
      Consumption = c.Consumption || false;
      let consumptionElement = document.getElementById("ConsumptionValue");
      consumptionElement.innerHTML = Consumption + "  &#13221;";
    }
    fetchconsumption();
    setInterval(fetchconsumption, 1000);

    async function fetchTime() {
      try {
        const r = await fetch(`${IPAdressESP}/time`);
      ...
      selected.clear();
      try {
        const r = await fetch(`${IPAdressESP}/getSchedule`);
        const d = await r.json();
      ...
      try {
        const r = await fetch(`${IPAdressESP}/pumpStatus.txt`);
        //  <!-- } } catch (e) { } } async function fetchPumpStatus() { try { const r = await fetch("/pumpStatus"); !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! -->
        const d = await r.json();
        pumpStatus = d.status;
      ...
      try {
        ...
        const r = await fetch(`${IPAdressESP}/pumpControl`, { method: "POST", headers: { "Content-Type": "application/json", }, body: JSON.stringify({ action: action, hours: hours, minutes: minutes, }), });
        const d = await r.json();
        if (d.success) {
      ...
      await fetch(`${IPAdressESP}/resync`, { method: "POST", });
      await init();
      ...
      await fetch(`${IPAdressESP}/setTime`, { method: "POST", headers: { "Content-Type": "application/json", }, body: JSON.stringify({ epoch, }), });
      await init();
      ...
      if (selected.size === 0) {
        ...
        await fetch(`${IPAdressESP}/clearSchedule`, { method: "POST", });
    ...
    document.getElementById("plan").addEventListener("click", async () => {
      ...
      for (const k of selected) {
        ...
        const res = await fetch(`${IPAdressESP}/saveSchedule`, { method: "POST", headers: { "Content-Type": "application/json", }, body: JSON.stringify({ entries: arr, }), });
    ...
    } const audioStates = { audio1: { playing: false, element: null, }, audio2: { playing: false, element: null, }, };
